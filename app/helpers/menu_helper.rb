module MenuHelper
  ...
  def menu_data
    ...
    # Menus principais
    menus_principais = [
      {id: :registration, icon: 'fa fa-file-text-o'},
    ]
    ...
    #Cadastros
    arr << menu_link(
      id: :index, text: I18n.t("activerecord.models.question.other"),
      icon: 'fa fa-question-circle', link: questions_path,
      controller: :questions,
      permission: can?(:index, Question), parent: :registration
    )
  ...
  def controller_name_sym
    controller_name = params.to_unsafe_h["controller"].to_sym
  end
  ...
  def submenu_active(submenu)
    ...
    (controller_name_sym == controller && (params.to_unsafe_h[:action].to_sym == submenu[:id] || submenu[:parent]==:registration)) ? "active" : ""
  end
